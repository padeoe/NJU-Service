...
public class OfflineQueryService implements DetailQuery{
    ...
    private String settingsBrasIP=null;
    ...
    public String getOnline(int page, int row){
        ...
        String result=NetworkUtils.postWithCookie("page="+page+"&rows="+row,cookie,"http://"+getBrasIP()+":8080/manage/self/online/getlist",timeout);
        if(isCookieInvalid(result)){
            resetCookie(authkey);
            result=NetworkUtils.postWithCookie("page="+page+"&rows="+row,cookie,"http://"+getBrasIP()+":8080/manage/self/online/getlist",timeout);
        }
        return result;
    }
    ...
    public String getAuthLog(int page, int row, boolean order) {
        ...
        String result=NetworkUtils.postWithCookie("sort=datetime&order="+(order?"desc":"asc")+"&page="+page+"&rows="+row,cookie,"http://"+getBrasIP()+":8080/manage/self/authlog/getlist",timeout);
        if(isCookieInvalid(result)) {
            resetCookie(authkey);
            result=NetworkUtils.postWithCookie("sort=datetime&order="+(order?"desc":"asc")+"&page="+page+"&rows="+row,cookie,"http://"+getBrasIP()+":8080/manage/self/authlog/getlist",timeout);
        }
        return result;
    }
    ...
    public String getAcct(int month,int page, int row, boolean order) {
        ...
        String result=NetworkUtils.postWithCookie("month="+month+"&sort=acctstarttime&order="+(order?"desc":"asc")+"&page="+page+"&rows="+row,cookie,"http://"+getBrasIP()+":8080/manage/self/detail/getlist",timeout);
        if(isCookieInvalid(result)) {
            resetCookie(authkey);
            result=NetworkUtils.postWithCookie("month="+month+"&sort=acctstarttime&order="+(order?"desc":"asc")+"&page="+page+"&rows="+row,cookie,"http://"+getBrasIP()+":8080/manage/self/detail/getlist",timeout);
        }
        return result;
    }
    ...
    public String getBills(int page, int row, boolean order) {
        ...
        String result=NetworkUtils.postWithCookie("sort=createtime&order="+(order?"DESC":"ASC")+"&page="+page+"&rows="+row,cookie,"http://"+getBrasIP()+":8080/manage/self/bill/getlist",timeout);
        if(isCookieInvalid(result)) {
            resetCookie(authkey);
            result=NetworkUtils.postWithCookie("sort=createtime&order="+(order?"DESC":"ASC")+"&page="+page+"&rows="+row,cookie,"http://"+getBrasIP()+":8080/manage/self/bill/getlist",timeout);
        }
        return result;
    }
    ...
    public String getRecharge(int page, int row, boolean order) {
        ...
        String result=NetworkUtils.postWithCookie("sort=oper_time&order="+(order?"DESC":"ASC")+"&page="+page+"&rows="+row,cookie,"http://"+getBrasIP()+":8080/manage/self/recharge/getlist",timeout);
        if(isCookieInvalid(result)) {
            resetCookie(authkey);
            result=NetworkUtils.postWithCookie("sort=oper_time&order="+(order?"DESC":"ASC")+"&page="+page+"&rows="+row,cookie,"http://"+getBrasIP()+":8080/manage/self/recharge/getlist",timeout);
        }
        return result;
    }
    ...
    public String getSettingsBrasIP() {
        return settingsBrasIP;
    }

    public void setSettingsBrasIP(String settingsBrasIP) {
        this.settingsBrasIP = settingsBrasIP;
    }
